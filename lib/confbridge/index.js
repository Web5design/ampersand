...
module.exports = function confbridge(db, ami, io, config) {
    ...
    this.create = function(req, res, next) {
        ...
        var expiration = date.getUTCFullYear() + '-' +
    ...
    this.load = function(req, res, next) {
        self.db.query('SELECT * FROM confbridge WHERE conference=?', req.params.conference, function(err, result) {
            ...
            if (result.length) {
                ...
                if (date) {
                    req.conference.expiration = date.getUTCFullYear() + '-' +
                        ('0' + (date.getUTCMonth() + 1)).slice(-2) + '-' +
                        ('0' + date.getUTCDate()).slice(-2);
                }
            } else {
    ...
    this.dashboard = function(req, res, next) {
        ...
        // Load the permanent conferences
        self.db.query('SELECT * FROM confbridge WHERE expiration IS NULL ORDER BY owner, conference ASC', function(err, result) {
            if (err) throw err;
            var permanent = result;

            // Load the active conferences
            self.db.query('SELECT * FROM confbridge WHERE expiration >= NOW()', function(err, result) {
                if (err) throw err;
                var active = result;

                // Load the expired conferences
                self.db.query('SELECT * FROM confbridge WHERE expiration < NOW()', function(err, result) {
                    if (err) throw err;
                    var expired = result;
                    ...
                    for (var i = 0; i < active.length; i++) {
                        active[i].expiration = active[i].expiration.getUTCFullYear() + '-' +
                            ('0' + (active[i].expiration.getUTCMonth() + 1)).slice(-2) + '-' +
                            ('0' + active[i].expiration.getUTCDate()).slice(-2);
                    }
                    for (var i = 0; i < expired.length; i++) {
                        expired[i].expiration = expired[i].expiration.getUTCFullYear() + '-' +
                            ('0' + (expired[i].expiration.getUTCMonth() + 1)).slice(-2) + '-' +
                            ('0' + expired[i].expiration.getUTCDate()).slice(-2);
                    }

                    // Populate the request object with the conferences.
                    req.confbridge = {
                        permanent: permanent,
                        active: active,
                        expired: expired
